...
#
#   nix-checks   -- 9 pure `nix build` targets (shared-crate tests, urp-cli,
#                   the two urp-bench cores, the urp-fast validators, and the
#                   4-kernel module matrix). Here they are BUILD-TIME deps of
...
let
  # The nine build targets of the ci.yml `nix-checks` job, in the same order.
...
pkgs.writeShellApplication {
  ...
  text = ''
    ...
    echo '======== job: nix-checks (9 build targets) ========'
    echo 'All build targets below were realised as dependencies of this app:'
${reportLines}
    echo 'nix-checks: 9/9 build targets realised (reaching here means they built).'
    ...
    echo "nix-checks: 9/9 build targets green"
