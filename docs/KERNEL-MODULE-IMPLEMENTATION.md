...
| 7d | Initiator multi-stream accept loop + concurrent-connection test | `74df057` | **Done.** `urp_accept_thread_fn` opens one stream per accepted UDS connection (per-stream pump emits SYN) instead of the single k0 `ep->conn`. Teardown fixes: shut UDS before `kthread_stop`; pump parks on `kthread_should_stop` after EOF (fixes `rmmod` hang + refcount oops). Validated by the microVM pair test's 12-concurrent-stream burst (KASAN/KMEMLEAK/lockdep CLEAN) and full Redpanda produce/consume over RDMA (`nix/test-redpanda-produce-consume.nix`, 11/11). Exercised via real concurrent connections rather than a `--stream-id` test-client. Follow-up: reap-on-close via FIN handshake (eager reap-on-`tx_done` reverted — dropped half-close responses). |
