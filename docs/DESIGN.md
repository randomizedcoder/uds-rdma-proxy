...
| 33 | [Initiator Connection Bring-up & Recovery](design/33-initiator-connection-bringup.md) | **Design.** An endpoint pair is a TCP-like socket over RDMA, but the initiator dials the RDMA-CM connection eagerly at `urp add` time with **no retry**, so a sub-second simultaneous-boot race with the acceptor's `rdma_listen` leaves the session permanently down (seen on hp1/hp3, design 32). Frames the fix by the TCP "connect on first use" analogy and recommends a **layered** design: bounded kernel connect-retry (the correctness floor), **lazy-connect on first UDS accept** (idiomatic + idle-clean), and a **userland gRPC control plane** (`UrpControl.Rendezvous`, extensible to param-negotiation/heartbeat/telemetry) for a deterministic rendezvous — after fixing two compounding recovery bugs (stale UDS socket `-98 EADDRINUSE`, acceptor QP-slot leak). Options table with pros/cons; v1 `.proto` sketch; the TCP≠RoCEv2-reachability caveat. |
