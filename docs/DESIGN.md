...
| 25 | [Provisioning Layer (outline)](design/25-provisioning-layer.md) | Trusted multi-cluster deployment (no kernel change): `urp-agent` host daemon reconciling tunnels via the `urp` CLI, shared bind-mount socket volumes host↔pod, Redpanda `unix_path` wiring, k8s DaemonSet + CRD/ConfigMap, PSK/bind allocation; open items: full produce/consume TCP bridging, inter-broker RPC, peer addressing |
...
| 28 | [Testability Review + Table-Driven-Test Plan](design/28-testability.md) | Why the §27.8 bugs lived in untestable code; current coverage (30 KUnit + ~71 Rust tests, almost none table-driven); extract-pure-function points (E1 frame classifier out of urp_recv_done, E2 stream SYN/FIN/RST state machine out of the handlers — the big one, with no Rust twin today, E3 netlink config-build); standardize a case-array idiom; phased plan; a real `nix run .#kunit` target |
