...
| 32 | [Real-Hardware RoCEv2 Integration Testing](design/32-real-hardware-integration-testing.md) | **In progress.** First end-to-end test off emulated `rdma_rxe` onto real **ConnectX-4 Lx 25 GbE** RoCEv2, across two back-to-back NixOS boxes (hp1 acceptor / hp3 initiator, `10.10.2.0/29`, port 4791). Makes `urp.ko` a pinned flake input via a reusable `nixosModules.urp` (builds against the host kernel, loads the RDMA stack, declares a systemd-managed endpoint pair); wires it into the hp1/hp3 configs (github rev + narHash pin, `make sync` → ssh `make`); then runs the client-combination **matrix** C↔C / C↔Rust / Rust↔C / Rust↔Rust (design 30 `urp-bench` twins, `--verify full`, `BENCH_OK` scrape) plus RDMA-direct `urp-test-client` and the design-31 `urp-fast` `/dev/urp` path — driven from workstation `l` by a new `nix run .#urp-hw-matrix`. Seastar row deferred (blocked on 31a). Blocking gate: urp.ko must build on the box's custom net-next kernel. Tracked in [32-implementation-status.md](design/32-implementation-status.md). |
| 33 | [Initiator Connection Bring-up & Recovery](design/33-initiator-connection-bringup.md) | **Design.** An endpoint pair is a TCP-like socket over RDMA, but the initiator dials the RDMA-CM connection eagerly at `urp add` time with **no retry**, so a sub-second simultaneous-boot race with the acceptor's `rdma_listen` leaves the session permanently down (seen on hp1/hp3, design 32). Frames the fix by the TCP "connect on first use" analogy and recommends a **layered** design: bounded kernel connect-retry (the correctness floor), **lazy-connect on first UDS accept** (idiomatic + idle-clean), and a **userland gRPC control plane** (`UrpControl.Rendezvous`, extensible to param-negotiation/heartbeat/telemetry) for a deterministic rendezvous — after fixing two compounding recovery bugs (stale UDS socket `-98 EADDRINUSE`, acceptor QP-slot leak). Options table with pros/cons; v1 `.proto` sketch; the TCP≠RoCEv2-reachability caveat. Tracked in [33-implementation-status.md](design/33-implementation-status.md). |
