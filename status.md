...
| **5** — MicroVM integration | `docs/KERNEL-MODULE-PLAN.md` §5 + `KERNEL-MODULE-IMPLEMENTATION.md` §Phase 5 | ✅ **x86_64 pair test PASS in ~38 s (`705ae8c`). Steps: 1 microvm.nix harness (`042982a`), 2 finer-grained verification phases (`1703d72`), 3 deferred rdma_connect fix (`2e8b5ce`), 4 socat-stay-alive workaround (`662b0af`), 5 pump half-close + drain order (`6dbac33`), 6 drop expect-eof wait (`36236f1`, 20-44x per-step speedup), 7 park TX pump on EOF (`705ae8c`, fixes Linux 7.0.3 kthread_stop crash; teardown 75s -> 2.2s). Total test 330s -> 38s (9x). aarch64/riscv64/sanitizer/CI/Redpanda deferred.** |
