...
| 39 | Prometheus metrics exporter | 🟢 **PR1 (MVP) + PR2 (deploy) + PR3 (aggressive verification) done; deployed to hp1/hp2/hp3.** `crates/urp-exporter` scrapes the genl `urp` counters (1 dump + N verbose GETs), renders `/metrics` by hand (no tokio/hyper/serde) from a single blocking thread; hardened CPU/Mem-capped `nixosModules.urp-exporter` (DynamicUser, CPUQuota 10%, MemoryMax 64M) **live on all 3 mesh nodes at ~650–740 KiB RSS**, scraped by l's Prometheus + Grafana (`urp-exporter-alerts.yml` 8 rules, `urp-exporter-dashboard.json` per-QP fairness split). **PR3**: mock-fleet source (`--features mock`) + `.#urp-exporter-stress` (100 Hz soak, 0 non-200, p99 <1 ms, RSS/fd flat), render hot path proven **0-alloc** (scratch-reuse refactor + counting-allocator unit test), 2 fuzz targets (`fuzz_http_request`/`fuzz_netlink_reply`, ~3.6M/2.2M execs clean), `cargo-audit` in the analysis tier (0 advisories). **PR4**: io_uring GET fan-out implemented (`urp-netlink::send_batch_uring`, feature-gated) + benchmarked on hp1 — **blocking wins at every N** (0.18× @N=1 … 0.98× @N=256; ring setup + kernel genl-fill dominate), so **blocking stays default, io_uring opt-in** (`urp-exporter-iouring`). <1% goodput-delta acceptance test/8h soak (PR5) pending |
