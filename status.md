...
| 39 | Prometheus metrics exporter | 🟢 **PR1 (MVP crate) + PR2 (deploy surface) merged.** `crates/urp-exporter` scrapes the genl `urp` counters (1 dump + N verbose GETs), renders `/metrics` by hand (no tokio/hyper/serde) from a single blocking thread; hardened, CPU/Mem-capped `nixosModules.urp-exporter` (DynamicUser, CPUQuota 10%, MemoryMax 64M, localhost-only); `nix/urp-exporter-alerts.yml` (8 rules — reorder-drops critical) + `nix/urp-exporter-dashboard.json` (per-QP fairness split). Stress/fuzz (PR3), io_uring fan-out (PR4), and hardware footprint acceptance test (PR5) pending |
